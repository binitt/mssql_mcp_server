...
- **`query`** tool for read-only SQL (Postgres-compatible API); **`execute_sql`** when writes are enabled
...
- **`mssql_readonly`** — Ask mode and Agent read queries; tool name **`query`** (`sql` argument, like Postgres).
...
By default the MCP server exposes a **`query`** tool (read-only): only `SELECT` / `WITH` (CTE) queries are accepted, and the tool is annotated with `readOnlyHint`. The API matches the official [`@modelcontextprotocol/server-postgres`](https://www.npmjs.com/package/@modelcontextprotocol/server-postgres) server (`query` tool, `sql` argument). With `MSSQL_ALLOW_WRITES=1`, the tool is named **`execute_sql`** and accepts all SQL.

| Variable | Default | Tool name | Effect |
|----------|---------|-----------|--------|
| `MSSQL_ALLOW_WRITES` | unset / `false` | `query` | SELECT-only; `readOnlyHint: true` |
