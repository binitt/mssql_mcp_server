    ...
    async def test_query_response_time(self):
        ...
        with patch('pymssql.connect', return_value=mock_conn):
            with patch.dict('os.environ', {
                ...
                result = await app.call_tool("read_query", {"query": "SELECT * FROM users"})
                end_time = time.time()
    ...
    async def test_concurrent_query_performance(self):
        ...
        with patch('pymssql.connect', return_value=mock_conn):
            with patch.dict('os.environ', {
                ...
                start_time = time.time()
                tasks = [
                    app.call_tool("read_query", {"query": f"SELECT COUNT(*) FROM table_{i}"})
                    for i in range(50)
                ]
    ...
    async def test_large_result_set_performance(self):
        ...
        with patch('pymssql.connect', return_value=mock_conn):
            with patch.dict('os.environ', {
                ...
                start_time = time.time()
                result = await app.call_tool("read_query", {"query": "SELECT * FROM large_table"})
                end_time = time.time()
    ...
    async def test_large_data_memory_handling(self):
        ...
        with patch('pymssql.connect', return_value=mock_conn):
            with patch.dict('os.environ', {
                ...
                result = await app.call_tool("read_query", {"query": "SELECT * FROM big_table"})
    ...
    async def test_burst_load_handling(self):
        ...
        with patch('pymssql.connect', return_value=mock_conn):
            with patch.dict('os.environ', {
                ...
                tasks = []
                for _ in range(100):
                    tasks.append(app.call_tool("read_query", {"query": "SELECT 1"}))
        ...
        with patch('pymssql.connect', return_value=mock_conn):
            with patch.dict('os.environ', {
                ...
                while time.time() - start_time < 10:
                    try:
                        result = await app.call_tool("read_query", {"query": "SELECT 'ok'"})
                        request_count += 1
        ...
        with patch('pymssql.connect', return_value=mock_conn):
            with patch.dict('os.environ', {
                ...
                for query in queries:
                    start_time = time.time()
                    result = await app.call_tool("read_query", {"query": query})
                    end_time = time.time()
